...
def get_kinematics_dfs(joint, result_list, is_root=True):
    ...
    if is_root:
        result_list.append(np.array([rot[1, 3]]))
    if joint.name != "End Site":
        result_list.append(rot[:3, :2].T.flatten())
    for child in joint.children:
        get_kinematics_dfs(child, result_list, is_root=False)

def get_data(motion, virtual_root, time_size=180, start_frame=0):
    data = []
    prev_yaw = None
    ...
    for i in range(start_frame, start_frame+time_size, 1):
        ...
        pose_features = np.concatenate(all_parts)
        
        final_feature = np.concatenate([linear_velocity, np.array([angular_velocity]), pose_features])
