...
def sample_motion_while_training(model, scheduler, mean, std, device, output_path, template_path, condition_pos, clip_length=180, feature_dim=211, guidance_scale=2.0):
    model.eval()
    ...
    # 안전 체크: [T, 2] shape인지 확인 (encode나 draw_trajectory에서 에러 방지)
    if trajectory_cloned.ndim != 2 or trajectory_cloned.shape[1] != 2:
        raise ValueError(f"Trajectory must be [T, 2] shape, but got {trajectory_cloned.shape}")

    encode(root, all_frames_data, output_filename=output_path, trajectory=trajectory_cloned)
