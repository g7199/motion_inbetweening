...
class MotionTransformer(nn.Module):
    def __init__(self, feature_dim=211, latent_dim=256, num_layers=8, ff_size=1024, nhead=4, dropout=0.1, activation="gelu", max_len=512, uncond_prob=0.1):
        super().__init__()
        ...
        self.pos_embed = nn.Sequential(
            nn.Linear(2, latent_dim//2),  # per-timestep: 2 -> latent_dim//2
            nn.SiLU(),
            nn.Linear(latent_dim//2, latent_dim)
        )

        self.pos_enc = PositionalEncoding(d_model=self.latent_dim, dropout=self.dropout, max_len=self.max_len + 1)

    def forward(self, x, con, t, force_unconditional=False):
        # x: [B, T, F], con: [B, T, 2], t: [B]
        B, T, F = x.shape
        device = x.device
        ...
        if force_unconditional:
            ...
            mask = mask.unsqueeze(1).unsqueeze(2).expand_as(con)  # [B, T, 2]로 expand
        ...
        con_emb = self.pos_embed(con.permute(1, 0, 2))  # [T, B, 2] -> [T, B, D]
        pos_emb = con_emb.mean(dim=0)  # [B, D] (평균으로 요약; 또는 .sum() / .max() 고려)

        combined_emb = (t_emb + pos_emb).unsqueeze(0)  # [1, B, D]
        x_emb = torch.cat([combined_emb, x_emb], dim=0)  # [T+1, B, D]
